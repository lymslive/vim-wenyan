...
if exists("b:current_syntax") && !exists('g:DEBUG')
  finish
endif
...
syntax match wenyanKeywordType /\v數|列|言|術|爻|物/
syntax match wenyanKeywordControl /\v恆為是|姑妄行此|如事不諧|乃作罷/
syntax match wenyanKeywordControl /\v若非|若|夫/
syntax match wenyanKeywordControl /\v乃得|乃歸空無|是謂|之術也|必先得|是術曰|乃行是術曰|欲行是術|也|云云|凡|恆為是|為是|遍|乃止/
syntax match wenyanKeywordOperator /\v等於|不等於|不大於|不小於|大於|小於/
syntax match wenyanKeywordOperator /\v加|減|乘|除|中有陽乎|中無陰乎|變|所餘幾何|之長|充|銜|其餘/
" syntax match wenyanKeywordOperator /\v昔之|今|是矣|其/
" syntax match wenyanKeywordOther /\v書之|名之曰|施|曰|噫|吾有|今有|有/
syntax match wenyanKeywordMacro /\v或云|蓋謂|嘗觀|方悟/

syntax match wenyanCommentLine /\v注曰|疏曰|批曰『.{-}』/
syntax match wenyanCommentLine /\v注曰|疏曰|批曰「「.{-}」」/

syntax match wenyanConstantsNumeric /\v零|一|二|三|四|五|六|七|八|九|十|百|千|萬|億|兆|京|垓|秭|穣|溝|澗|正|載|極|分|釐|毫|絲|忽|微|纖|沙|塵|埃|渺|漠|虛|空|清|淨/
syntax match wenyanConstantsBool /\v陰|陽/

" syntax match wenyanStringQuoted /\v「「.{-}」」/
syntax match wenyanStringQuoted /\v『.{-}』/
syntax match wenyanIdentifier /\v「.{-}」/
...
hi def link wenyanKeywordModifier      Keyword
hi def link wenyanIdentifier           Identifier
...
hi def link wenyanKeywordMacro         PreProc
